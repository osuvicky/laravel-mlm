          ...
          <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                  ...
                  @if ($permission->roles)
                      ...
                      <form class="btn btn-icon btn-danger" method="POST" action="{{route('admin.permissions.roles.remove', [$permission->id, $permission_role->id])}}" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-icon btn-danger" title="remove"><i class="fas fa-times"></i>  {{ $permission_role->name}}</button>
                                            </form>
                      @endforeach
                    @endif
                    </div>
